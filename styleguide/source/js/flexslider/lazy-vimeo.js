// Lazy Load Vimeo
// original by Kevin Weber (kevinw.de)
...
(function( lazyload_vimeo, $, undefined ) {
  ...
  var vimeoCreatePlayer = function() {
    $(classPreviewVimeoDot).on('click', function() {
      ...
      $(this).html('<iframe class="vimeo" src="' + vimeoUrl( vid ) + '?autoplay=1' + playercolour + '" style="height:' + (parseInt($("#" + vid).css("height"))) + 'px;width:100%" frameborder="0" webkitAllowFullScreen mozallowfullscreen autoPlay allowFullScreen></iframe>');
  ...
  var vimeoLoadingThumb = function(id) {
    $.getJSON(vimeoCallbackUrl(id) + ".json?callback=?", {
      format: "json"
      ...
      $("#" + id).prepend('<div style="height:' + (parseInt($("#" + id).css("height"))) + 'px;width:' + (parseInt($("#" + id).css("width"))) + 'px; background: #000 url(' + largeThumb + ') center center no-repeat" class="lazy-load-vimeo-div"><span class="titletext vimeo"'+ itemprop_name +'></span></div>').addClass($_o.buttonstyle);
